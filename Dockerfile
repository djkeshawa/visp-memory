# Multi-stage build for Visp Memory with Dashboard
...
RUN pip install --no-cache-dir "uv==0.11.7" && \
    ...
    pip install --no-cache-dir --constraint constraints.txt "$(ls *.whl)[${VISP_MEMORY_EXTRAS}]" && \
...
ENV VISP_MEMORY_STORAGE_DATA_DIR=/data
ENV VISP_MEMORY_EMBEDDING_PROVIDER=auto
