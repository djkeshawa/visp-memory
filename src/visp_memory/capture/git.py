...
class GitCapture:
    ...
    def __init__(self, memory, repo_path: Path = None):
        ...
        if not GIT_AVAILABLE:
            raise ImportError(
                "GitPython required for git capture. Install with: pip install visp-memory[capture]"
            )
    ...
    def uninstall_hooks(self) -> Dict[str, bool]:
        ...
        for hook_name in ["post-commit", "post-merge"]:
            ...
            if hook_path.exists():
                ...
                if "visp-memory capture" in content:
                    hook_path.unlink()
    ...
    def _install_hook(self, hook_path: Path, script: str) -> bool:
        ...
        try:
            ...
            if hook_path.exists():
                ...
                if "visp-memory capture" in existing:
                    # Already installed
                    return True
                else:
    ...
    def _generate_post_commit_script(self) -> str:
        ...
        return """#!/bin/sh
# visp-memory auto-capture hook
# Automatically records commits to memory

# Run visp-memory capture in background to avoid slowing down commits
(visp-memory capture git commit --ref HEAD &) 2>/dev/null
...
# Run visp-memory capture in background
(visp-memory capture git merge &) 2>/dev/null
