"""
Pluggable embedding providers for Visp Memory.
...
def _candidate_providers_for_auto(config: EmbeddingConfig) -> list[str]:
    ...
    Order matters: a configured cloud key is the strongest signal of intent, then a
    locally installed sentence-transformers model, then a reachable Ollama daemon.
    Every candidate is constructed with ``verify=True`` so an unreachable service
    raises here and we fall through rather than failing later on first recall.
    """
    providers = []
...
def _sentence_transformers_available() -> bool:
    """Report whether local sentence-transformers embeddings can be built."""
    ...
    if provider in {"auto", "cloud"}:
        for candidate in _candidate_providers_for_auto(config):
            try:
                ...
                # Falling through is the designed behaviour of auto-selection, not an
                # anomaly, so this is debug-level: a first run with no cloud key and no
                # Ollama daemon is the common case and must not look like a failure.
                # The single actionable warning is emitted once by _warn_noop_fallback()
                # if every candidate is exhausted.
                logger.debug(
...
_noop_fallback_warned = False


def _warn_noop_fallback() -> None:
    """Note once per process that recall is running on keyword search, not vectors.

    This is INFO, not WARNING, because it describes a supported configuration rather
    than a fault. Every storage backend detects noop embeddings and routes recall
    through its keyword/text path instead of trusting constant vectors, so results are
    still ranked -- they just lose semantic (synonym/paraphrase) matching. Emitting a
    scary warning here fired on the documented lean-install path, on every single CLI
    invocation, for behaviour that works as designed.
