...
| Independent keyword/vector discovery (`hybrid_union`) | Experimental | Opt-in SQLite/Neo4j candidate union before rank fusion. Existing ranking defaults are unchanged; answer-accuracy benefit is not yet measured. |
...
Capability execution and usefulness are separate checks:

| Capability | Integration contract | Measured benefit / limitation |
|---|---|---|
| Source-backed compression and reflection | Canonical source lookup, inherited runtime scope and weakest provenance; source citations survive reopen on SQLite and Neo4j. | Source integrity is tested. This does not establish better answers; reflection produces coding runbooks. |
| Coverage context selection | Verbatim passages retain speaker/section context and source offsets within the rendered budget. | Experimental and opt-in; answer quality requires matched evaluation. |
| Proactive recall | Complete evidence units are included or explicitly omitted; JSON remains valid at supported budgets. | Prevents clipped qualifications; automatic injection retains its specificity gates. |
| HTTP answer evidence | Reader receives bounded source passages; UI previews remain separate. | Prevents preview truncation from hiding evidence; reader reasoning remains a separate limit. |
| Neo4j diagnostics | Intent usage is inspected without schema initialization; failures are reported as unavailable. | Describes store usage, not quality or readiness. |
