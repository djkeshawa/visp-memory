...
Visp Memory includes an MCP server so compatible AI assistants can read and write
...
Use the CLI installer when Codex should connect to a running visp-memory server:
...
The installer adds a managed `mcp_servers.visp-memory` block to the Codex config
...
```json
{
  ...
  "files": ["src/visp_memory/server/auth_store.py"],
...
```json
{
  "area": "src/visp_memory/server/app.py",
...
python3 -c "from visp_memory.interfaces.mcp import MCP_AVAILABLE; print(MCP_AVAILABLE)"
```
