...
This document describes how LLM Memory should be operated when project memory
contains engineering decisions, team context, or captured development activity.

## Stored Data

LLM Memory stores memory content, categories, importance, tags, metadata,
...
Use `llm-memory list`, `llm-memory recall`, and dashboard views to inspect
...
Server deployments should configure `LLM_MEMORY_JWT_SECRET`, API keys, and
