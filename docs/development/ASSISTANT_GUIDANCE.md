...
```bash
llm-memory hooks install codex --repo-id <repo-id>
llm-memory hooks update codex --task "<task>" --file <path>
```

The managed `AGENTS.md` section tells assistants to:

- recall the latest memory with `llm-memory remember --repo <repo-id>` or MCP
  `memory_remember`;
- search project memory with `llm-memory recall "<task>" --repo <repo-id>` or
  MCP `memory_recall`;
- inspect file-specific context with `llm-memory inject --file <path>
  --task "<task>"` or MCP `memory_file_context`;
- record meaningful outcomes with `llm-memory record`, `llm-memory decision`,
...
```bash
llm-memory hooks uninstall codex
```
