...
surface that llm-memory provides before a model answers.
"""
...
CASES: tuple[EvalCase, ...] = (
    EvalCase(
        ...
        question="Do we need the llm-memory Codex plugin and MCP server, or just one?",
        answerable=True,
        memories=[
            SeedMemory(
                "Decision: Keep the llm-memory plugin as the Codex packaging layer "
    ...
    EvalCase(
        ...
        memories=[
            SeedMemory(
                ...
                "The CLI equivalent is llm-memory remember.",
                layer="episodic",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate hallucination-risk reduction from llm-memory grounding."
    )
...
def main() -> None:
    ...
    if args.data_dir:
        ...
        with tempfile.TemporaryDirectory(prefix="llm-memory-hallucination-eval-") as tmpdir:
