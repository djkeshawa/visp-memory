...
4. In MCP mode, call `memory_prepare_task` with the task and repo ID for a cited brief, and `memory_recall` for search. These are available in the default core and readonly profiles. `memory_goal` and `memory_working_on` set direction in core/full profiles; `focus` is CLI-only. For the latest recorded memory, use the CLI command above.
