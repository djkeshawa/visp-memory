---
name: llm-memory-recall
description: Recall LLM Memory context for the current repository before coding, debugging, reviewing, or planning. Use local direct mode first through the `llm-memory` CLI or Python package; server/API mode is optional.
---
...
2. Prefer local direct mode:
   - Run `llm-memory status` to confirm a local config exists.
   - Run `llm-memory remember --repo <repo-id>` when the user asks what the AI most recently remembered.
   - Run `llm-memory recall "<task>" --repo <repo-id>` when a repo id is known.
   - Run `llm-memory context --task "<task>"` when broader context is needed.
